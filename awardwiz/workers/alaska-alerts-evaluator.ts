...
import { memoizeAlaskaSearch, searchAlaska } from "../backend/alaska-alerts/alaska-search.js"
...
const memoizedSearchAlaska = memoizeAlaskaSearch(searchAlaska)
...
await new Listr(dueAlerts.map((alert) => ({
  ...
  task: async () => evaluateOneAlert({
    alert,
    repository,
    searchAlaska: memoizedSearchAlaska,
    now: new Date(),
  }),
})), {
  concurrent: 3,
