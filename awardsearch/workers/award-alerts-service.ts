...
export const startAwardAlertsService = async ({
  ...
  const evaluatorLoop = createLoopRunner({
    ...
    run: async () => await runEvaluatorWorker({
      databasePath,
      repository,
      providers,
      now: new Date(),
  ...
  const notifierLoop = createLoopRunner({
    ...
    run: async () => await runNotifierWorker({
      databasePath,
      repository,
      fetchFn,
      webhookUrl,
  ...
  const service = createAwardAlertsService({
    ...
    runEvaluator: async () => await evaluatorLoop.trigger("manual"),
    runNotifier: async () => await notifierLoop.trigger("manual"),
  })
