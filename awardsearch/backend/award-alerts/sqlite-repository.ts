...
const mapAlertRow = (row: AwardAlertRow): AwardAlert => {
  ...
  const dateMode = row.date_mode as string
  invariant(dateMode === "date_range", `award alert ${row.id} has unsupported date_mode ${row.date_mode}`)
...
export class SqliteAwardAlertsRepository {
  ...
  public getAlert(id: string): AwardAlert | undefined {
  ...
  public updateAlert(alert: AwardAlert) {
  ...
  public insertAlert(alert: AwardAlert) {
    this.db.prepare(`
  ...
  public listAlertRuns(alertId: string): AwardAlertRun[] {
    return (this.db.prepare(`
      SELECT *
  ...
  public listNotificationEvents(alertId: string): NotificationEvent[] {
    return (this.db.prepare(`
      SELECT *
  ...
  public getState(alertId: string): AwardAlertState | undefined {
  ...
  public pauseAlert(id: string, updatedAt: string) {
    const result = this.db.prepare(`
      UPDATE award_alerts
  ...
  public resumeAlert(id: string, updatedAt: string) {
    const result = this.db.prepare(`
      UPDATE award_alerts
  ...
  public saveEvaluation({ alert, state, run }: { alert: AwardAlert, state: AwardAlertState, run: AwardAlertRun }) {
    invariant(
  ...
  public createNotificationEvent(event: NotificationEvent) {
    this.db.prepare(`
  ...
  public claimDueAlerts(nowIso: string, limit: number, claimTtlMinutes: number): AwardAlert[] {
  ...
  public claimPendingNotificationEvents(limit: number, claimedAt: string, staleBefore: string): NotificationEvent[] {
    return this.db.transaction(() => {
  ...
  public markNotificationAttempting(id: string, attemptedAt: string, claimToken: string | undefined) {
    this.db.transaction(() => {
  ...
  public markNotificationDeliveredUnconfirmed(id: string, reason: string, claimToken?: string) {
    this.db.transaction(() => {
      const current = this.db.prepare("SELECT status, claim_token, attempted_at FROM notification_events WHERE id = ?").get(id) as {
        status: NotificationEventStatus
        claim_token: string | null
  ...
  public markNotificationSent(id: string, sentAt: string, claimToken?: string) {
    this.db.transaction(() => {
      const current = this.db.prepare("SELECT status, claim_token, attempted_at FROM notification_events WHERE id = ?").get(id) as {
        status: NotificationEventStatus
        claim_token: string | null
  ...
  public markNotificationFailed(id: string, reason: string, claimToken?: string) {
    this.db.transaction(() => {
      const current = this.db.prepare("SELECT status, claim_token, attempted_at FROM notification_events WHERE id = ?").get(id) as {
        status: NotificationEventStatus
        claim_token: string | null
