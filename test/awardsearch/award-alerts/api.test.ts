...
import { createAwardAlertsApp } from "../../../awardsearch/backend/award-alerts/server.js"
...
const createService = (overrides: Partial<AwardAlertsHttpService> = {}): AwardAlertsHttpService => ({
  listAlerts: vi.fn().mockResolvedValue([alert]),
  ...
  pauseAlert: vi.fn().mockResolvedValue({ ...alert, active: false }),
  resumeAlert: vi.fn().mockResolvedValue({ ...alert, active: true }),
  deleteAlert: vi.fn().mockResolvedValue(alert),
  previewAlert: vi.fn().mockResolvedValue(preview),
  getAlertRuns: vi.fn().mockResolvedValue([{ id: "run-1" }]),
  getAlertNotifications: vi.fn().mockResolvedValue([{ id: "event-1" }]),
  getStatus: vi.fn().mockReturnValue(status),
  triggerEvaluatorRun: vi.fn().mockResolvedValue({ started: "evaluator" }),
  triggerNotifierRun: vi.fn().mockResolvedValue({ started: "notifier" }),
  runScraperBatch: vi.fn().mockResolvedValue(rawScraperBatchResult),
  ...overrides,
})
...
describe("award alerts HTTP API", () => {
  ...
  it("supports browser preflight requests for the raw scraper endpoint", async () => {
    const service = createService({
      ...
      getAlert: vi.fn().mockResolvedValue(undefined),
      getAlertRuns: vi.fn().mockResolvedValue([]),
      getAlertNotifications: vi.fn().mockResolvedValue([]),
      triggerEvaluatorRun: vi.fn().mockResolvedValue({ started: true }),
      triggerNotifierRun: vi.fn().mockResolvedValue({ started: true }),
    })
  ...
  it("maps not found and bad request errors to a stable JSON shape", async () => {
    const service = createService({
      listAlerts: vi.fn().mockResolvedValue([]),
      getAlert: vi.fn().mockResolvedValue(undefined),
      createAlert: vi.fn().mockRejectedValue(new Error("unsupported award program: aeroplan")),
      updateAlert: vi.fn().mockRejectedValue(new Error("award alert not found: alert-404")),
      pauseAlert: vi.fn().mockRejectedValue(new Error("award alert not found: alert-404")),
      resumeAlert: vi.fn().mockRejectedValue(new Error("award alert not found: alert-404")),
      deleteAlert: vi.fn().mockRejectedValue(new Error("award alert not found: alert-404")),
      previewAlert: vi.fn().mockRejectedValue(new Error("Invalid value for origin")),
      getAlertRuns: vi.fn().mockResolvedValue([]),
      getAlertNotifications: vi.fn().mockResolvedValue([]),
      triggerEvaluatorRun: vi.fn().mockRejectedValue(new Error("scheduler offline")),
      triggerNotifierRun: vi.fn().mockRejectedValue(new Error("scheduler offline")),
      runScraperBatch: vi.fn().mockRejectedValue(new Error("unsupported scraper: skyscanner")),
    })
    ...
    expect(response.response.status).toBe(404)
    const alertNotFoundBody = (response.body as { error: { code: string } })
    expect(alertNotFoundBody.error.code).toBe("alert_not_found")
