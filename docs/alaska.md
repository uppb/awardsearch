...
This document describes the Alaska scraper in [`awardwiz-scrapers/scrapers/alaska.ts`](../awardwiz-scrapers/scrapers/alaska.ts).
...
- Converts that payload into AwardWiz's shared `FlightWithFares` format.
...
- Domestic or Alaska-specific cabin naming is flattened into AwardWiz's simpler cabin model; there is no separate premium-economy representation.
