...
import { AwardSearchScraperModule, DatedRoute, FlightFare, FlightWithFares } from "./awardsearch-types.js"
...
const runQuery = async (scraperName: string, route: string[], checkDate = dayjs().add(3, "months").format("YYYY-MM-DD")) => {
  const scraper = await import(`./scrapers/${scraperName}.ts`) as AwardSearchScraperModule
...
if (scrapers.length === 0) {
  describe("awardsearch scrapers", () => {
    it("has no active scraper integration cases configured", () => {
      expect(scrapers).toHaveLength(0)
    })
  })
}
else {
